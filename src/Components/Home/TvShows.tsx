...
function TvShows({ type, genres }: { type: string; genres?: IGenres[] }) {
  const navigate = useNavigate();
  const bigMovieMatch = useMatch("/tv/:tvId");
  const { scrollY } = useScroll();
  const { data: airingToday } = useQuery<IGetTvResult>(
    ["tvShows", "airingToday"],
    getAiringTodayTv
  );
  ...
  const data =
    type === "popular" ? popular : type === "topRated" ? topRated : airingToday;
  ...
  const clickedMovie =
    ...
    data?.results.find((tv) => tv.id === Number(bigMovieMatch?.params.tvId));
  return (
    <>
      {type === "airingToday" && (
